...
void GenerateA78CRC(const char *pFilename)
{
	u8 aHeader[128];
	...
	if (fread(aHeader, 1, 128, file) != 128 ||
		aHeader[0] < 1 || aHeader[0] > 4 || memcmp(aHeader + 1, "ATARI7800", 9) != 0)
	{
		fclose(file);
		printf("%s : invalid A78 file.\n", pFilename);
		return;
	}
